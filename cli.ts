...
async function main() {
  try {
    if (args.help || args.h) {
      console.log(
        chalkin.bold.underline(`${NAME}@${VERSION}`) + "\n" +
      ...
      console.log(
        "  -o, --optimize          Optimize your WASM during compilation (can only be used with the `execute` flag)",
      );
      console.log(
        "  --outFile               The output file to write the `.wasm` at (must be used with `compile` flag) ",
      );
      console.log(
        "  --compile               Compile a `.wat` file to `.wasm` file",
      );
