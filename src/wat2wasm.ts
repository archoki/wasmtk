...
import { stripComments, maxMemoryCheck } from './internal//utils.ts';
 ...
 */
export async function compile(wat: string, options?: WASMTK.CompilerOption): Promise<Uint8Array> {
  ...
  if (options && Object.keys(options).length !== 0) {
    for (const option in options) {
      defaultOptions[option as keyof WASMTK.CompilerOption] = options[option as keyof WASMTK.CompilerOption];
    }
  }
